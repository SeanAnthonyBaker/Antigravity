-- ROBUST FIX ADMIN ACCESS SCRIPT v2
...
DROP FUNCTION IF EXISTS public.get_all_users_with_approval();
DROP FUNCTION IF EXISTS public.approve_user(UUID);

-- 3. Re-create 'get_all_users_with_approval' with explicit search_path
...
-- 4. Re-create 'approve_user' with explicit search_path
...
GRANT EXECUTE ON FUNCTION public.get_all_users_with_approval() TO service_role;
GRANT EXECUTE ON FUNCTION public.approve_user(UUID) TO service_role;

-- 6. Ensure 'user_roles' policies allow admins to read/update
-- Drop restrictive policies if they exist to be safe, then re-add
...
-- 7. Explicitly Grant Admin Access to 'seanbaker513@gmail.com'
DO $$
DECLARE
    target_uid UUID;
BEGIN
