...
async function run() {
    try {
        ...
        await client.query('UPDATE documents SET url = $1, urltype = $2 WHERE "nodeID" = 617', [testUrl, 'InfoGraphic']);
        const res = await client.query('SELECT "nodeID", title, url, urltype FROM documents WHERE "nodeID" = 617');
        console.log('Successfully updated node 617:');
